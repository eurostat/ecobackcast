...
countryNum = 15
quarter = rep(c("Q1","Q2","Q3","Q4"), each = countryNum)
country = rep(EUregions[["EU15"]],4)

gc <- rbind(quarter,country)
rownames(gc) <- c("Quarter", "Country")
...
dataQ    <- window(dataQ, end = 2017.75 )
dataAnn  <- window(dataAnn, end = 2017.75)
...
startIndexA  <- which(time(dataAnn) == startDateQ) 
...
f        <- c(1,4,rep(1,15), rep(4,15))

models = NULL
library(forecast)

## Reverse all of the time series and make sure we preserve the seasonality
for (i in 1:length(dataTemp)){
  dataTemp[[i]] <- ts(rev(dataTemp[[i]]), frequency = f[i])
}

# Either Backcast each of the time series, or select the truth
years    =  as.vector(sapply(dataTemp, FUN = numNA)/f)
len      =  sapply(dataTemp, FUN = numNAnot)
maxYears =  max(years)

backcastModels = list(rep(NA, length(dataTemp)))
backcasts      = list(rep(NA, length(dataTemp)))
for (i in 1:length(dataTemp)){
  backcastModels[[i]] <-
    auto.arima(
...
data.f <- buildHier(data.f, type = "Q")
...
# Divide annual back through by 4
dataFinalA <- dataFinalA/4
...
# Write to Excel
write.xlsx(x = list("Annual" = dataFinalA,"Quarterly" = dataFinalQ), file = "../Database/employmentBackcast.xlsx", rowNames = T)
