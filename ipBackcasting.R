...
weights.ip <- weights.ip[colnames(data)[-c(1,2)]]
...
smat[1,] <- as.vector(unlist(weights.ip[,EUregions["EU15"][[1]]][1,]))
...
dataFinal[which(is.na(dataFinal[, 2])), 2] <- rowSums(sweep(
  ...
  MARGIN = 2,
  STATS =  unlist(weights.ip[EUregions[["EA12"]]][2,]),
  FUN = "*"))
